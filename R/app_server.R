...
loupe_image_paths <- tibble("LoupeImages" = list.files(path = "www/ExamVisionLoupeImages/"))
...
examvision_data <- googlesheets4::read_sheet(sheet_id, sheet = "Loupe_types", col_types = "c") %>%
  filter(`Mfg` == 'ExamVision') %>%
  rename(`ExamVision Frame` = Mod, Style = Size, `Innovative Optics Insert` = `Insert Part Number` )
...
app_server <- function(input, output, session) {
  ...
  loupe_insert <- eventReactive(c(input$loupestyle ),{
    result <- examvision_data %>%
      filter(`ExamVision Frame` == input$loupestyle) %>%
  ...
  user_info <- eventReactive(input$run,{
    result <- tibble(
      "ExamVision Loupe Style" = loupe_insert()$`ExamVision Frame`,
  ...
  image_location <- eventReactive(input$run,{
    ...
    result <- c(glue::glue_safe("www/ExamVisionLoupeImages/", loupe_rec_front),
                glue::glue_safe("www/ExamVisionLoupeImages/", loupe_rec_side),
                glue::glue_safe("www/ExamVisionLoupeImages/", loupe_rec_top),
