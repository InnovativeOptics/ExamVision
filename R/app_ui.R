...
# theming options
examvision_theme <- bs_theme(version = 5,
    ...
    page_fluid(
      ...
      titlePanel("",
                 tags$head(tags$link(rel = "icon", type = "png", href = "icons/logo-small.png"),
                           tags$title("ExamVision Laser Safety"))),

      theme = examvision_theme,
      card(card_header(inverse = T,
                       fluidRow(
                         #was 6, but may need to change back to 6 if they want to add customer service info
                         column(12,
                                align = 'center',
...
#                         )
      ...
      fluidRow(
        column(
          ...
          selectInput(
            inputId = "loupestyle",
            label = h4(strong("ExamVision Frame")),
            choices = sort(examvision_data$`ExamVision Frame`),
            selected = 1
          )
        ),
#        column(
...
golem_add_external_resources <- function() {
  ...
  #  favicon(),
    bundle_resources(
      ...
      app_title = "ExamVision Laser Safety"
    ),
